...
float dx(int i, float *t, float Tens, float Bias, float Cont, Curve *Fig) {
    ...
    if (i % 2 == 0)
        return 0.5 * (1 - Tens) * (1 + Bias) * (1 + Cont) * (Fig->CP.at(i).x - Fig->CP.at(i - 1).x) / (t[i] - t[i - 1]) +
               0.5 * (1 - Tens) * (1 - Bias) * (1 - Cont) * (Fig->CP.at(i + 1).x - Fig->CP.at(i).x) / (t[i + 1] - t[i]);
    else
        return 0.5 * (1 - Tens) * (1 + Bias) * (1 - Cont) * (Fig->CP.at(i).x - Fig->CP.at(i - 1).x) / (t[i] - t[i - 1]) +
               0.5 * (1 - Tens) * (1 - Bias) * (1 + Cont) * (Fig->CP.at(i + 1).x - Fig->CP.at(i).x) / (t[i + 1] - t[i]);
}
float dy(int i, float *t, float Tens, float Bias, float Cont, Curve *Fig) {
    ...
    if (i % 2 == 0)
        return 0.5 * (1 - Tens) * (1 + Bias) * (1 + Cont) * (Fig->CP.at(i).y - Fig->CP.at(i - 1).y) / (t[i] - t[i - 1]) +
               0.5 * (1 - Tens) * (1 - Bias) * (1 - Cont) * (Fig->CP.at(i + 1).y - Fig->CP.at(i).y) / (t[i + 1] - t[i]);
    else
        return 0.5 * (1 - Tens) * (1 + Bias) * (1 - Cont) * (Fig->CP.at(i).y - Fig->CP.at(i - 1).y) / (t[i] - t[i - 1]) +
               0.5 * (1 - Tens) * (1 - Bias) * (1 + Cont) * (Fig->CP.at(i + 1).y - Fig->CP.at(i).y) / (t[i + 1] - t[i]);
}

float DX(int i, float *t) { return Derivata.CP.at(i).x == 0 ? dx(i, t, 0.0, 0.0, 0.0, &Poligonale) : Derivata.CP.at(i).x; }
float DY(int i, float *t) { return Derivata.CP.at(i).y == 0 ? dy(i, t, 0.0, 0.0, 0.0, &Poligonale) : Derivata.CP.at(i).y; }
...
void hermiteInterpolation(float *t, glm::vec4 color_top, glm::vec4 color_bot, Curve *curve) {
    ...
    for (tg = 0; tg <= 1; tg += passotg) {
        ...
        x = curve->CP[is].x * PHI0(tgmapp) + DX(is, t) * PHI1(tgmapp) * ampiezza + curve->CP[is + 1].x * PSI0(tgmapp) +
            DX(is + 1, t) * PSI1(tgmapp) * ampiezza;
        y = curve->CP[is].y * PHI0(tgmapp) + DY(is, t) * PHI1(tgmapp) * ampiezza + curve->CP[is + 1].y * PSI0(tgmapp) +
            DY(is + 1, t) * PSI1(tgmapp) * ampiezza;
...
void buildHermite(glm::vec4 color_top, glm::vec4 color_bot, Shared<Curve> curve) {
    ...
    if (curve->CP.size() > 1) {
        ...
        float step = 1. / (float)(curve->CP.size() - 1);

        for (int i = 0; i < curve->CP.size(); i++) {
            t[i] = (float)i * step;
        }

        hermiteInterpolation(t, color_top, color_bot, curve.get());
        curve->nv = curve->vertex.size();
    }
